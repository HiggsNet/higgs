...
func TestFirewallBackendsRootSmoke(t *testing.T) {
	...
	if _, err := exec.LookPath("iptables"); err == nil {
		...
		t.Log("iptables not found; skipping iptables backend root smoke")
	}
}
...
func runFirewallBackendRootSmoke(t *testing.T, ctx context.Context, nsName, backend string) {
	...
	assertFirewallBackendState(t, ctx, nsName, backend, ownerPrefix, spec.NetNS, []string{"10.42.0.1", "10.43.0.0/24"}, []string{"10.99.0.0/24"})
	...
	assertFirewallBackendState(t, ctx, nsName, backend, hostOwnerPrefix, "host", []string{"1500", "14500"}, nil)
}
...
func firewallBackendDump(t *testing.T, ctx context.Context, nsName, backend, ownerPrefix, scope string) string {
	...
	case BackendNFT:
		...
		return string(out)
	case BackendIptables:
		out, err := exec.CommandContext(ctx, "ip", "netns", "exec", nsName, "iptables", "-S").CombinedOutput()
		if err != nil {
			t.Fatalf("iptables -S: %v\noutput: %s", err, string(out))
		}
		nat, _ := exec.CommandContext(ctx, "ip", "netns", "exec", nsName, "iptables", "-t", "nat", "-S").CombinedOutput()
		var lines []string
