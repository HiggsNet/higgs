...
func TestStateGCOnlyPlansUnconfiguredBirdInstances(t *testing.T) {
	...
	config.Routing = routingConfig{Instances: []RoutingInstance{
		{ID: "main", NetNS: "h2", Enabled: true, Mode: ipsec.RoutingModeManaged},
	...
	if len(state.BirdInstances) != 2 || state.BirdInstances["h2"] == nil || state.BirdInstances["edge"] == nil {
...
func TestStateGCEmptyPlanDoesNotMutateState(t *testing.T) {
	config := defaultAppConfig()
	config.Routing = routingConfig{Instances: []RoutingInstance{{ID: "main", NetNS: "h2", Enabled: true}}}
	state := &stateFile{BirdInstances: map[string]*BirdInstanceState{"h2": {NetNSName: "h2"}}}
...
func TestDaemonStateGCApplyPersistsPlan(t *testing.T) {
	...
	appConfig.Routing = routingConfig{Instances: []RoutingInstance{{ID: "main", NetNS: "h2", Enabled: true}}}
	...
	if reloaded.BirdInstances["h2"] == nil {
		t.Fatalf("configured h2 BIRD state was removed: %#v", reloaded.BirdInstances)
	}
}
