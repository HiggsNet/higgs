...
func TestDebugHelpOnlyAdvertisesRoutingEntryPoint(t *testing.T) {
	...
	if err := command.Run(context.Background(), []string{"higgs", "debug", "--help"}); err != nil {
...
filter higgs_import_main {
    if net ~ [ 10.0.0.0/8+ ] then accept;
    reject;
}

filter higgs_export_main {
...
protocol babel higgs_babel_main {
	...
	for _, want := range []string{"filter higgs_import_main", "10.0.0.0/8+", "filter higgs_export_main", "10.1.0.0/24"} {
