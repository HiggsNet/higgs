...
func linkOutputsFromState(state *stateFile) []higgsstate.LinkOutput {
	if state == nil {
		return nil
	}
	...
	out := make([]higgsstate.LinkOutput, 0, len(ids)*2)
	for _, id := range ids {
		...
		for i, want := range wants {
			...
			staged := base
			staged.ID = runtimeLinkOutputID(firstNonEmpty(inst.LinkID, inst.ID, want.LinkID, want.InstanceID), higgsstate.LinkRuntimeStaged)
			...
			staged.RuntimeRole = higgsstate.LinkRuntimeStaged
			staged.Endpoint = ""
...
func newIPsecLinkOutput(inst linkInstanceState, desired desiredLinkState) higgsstate.LinkOutput {
	...
	return higgsstate.LinkOutput{
		...
		RuntimeRole:    higgsstate.LinkRuntimeActive,
...
func runtimeLinkOutputID(linkID, role string) string {
	if role == "" || role == higgsstate.LinkRuntimeActive {
		return linkID
	}
	return linkID + "#" + role
}

func baseLinkReadiness(state, iface string) higgsstate.LinkReadiness {
	ready := higgsstate.LinkReadiness{Session: higgsstate.LinkReadyNotReady, Interface: interfaceReadiness(iface, state), Routing: higgsstate.LinkReadyUnknown, Health: higgsstate.LinkReadyUnknown}
	if state == "up" {
		ready.Session = higgsstate.LinkReadyReady
	}
	return ready
}
...
func interfaceReadiness(iface, state string) string {
	if iface != "" && state != "" && state != "removing" && state != "failed" {
		return higgsstate.LinkReadyReady
	}
	return higgsstate.LinkReadyNotReady
}
