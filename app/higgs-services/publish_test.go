...
func TestPublishResolvedServiceOrdersReadinessACLRouteAndRecord(t *testing.T) {
	...
	logPath := filepath.Join(t.TempDir(), "higgs.log")
	binary := writeFakeHiggs(t, logPath)
	...
	if err != nil {
		t.Fatalf("read fake Higgs log: %v", err)
	}
	...
	if len(got) != len(want) || strings.Join(got, "\n") != strings.Join(want, "\n") {
		t.Fatalf("Higgs calls = %q (fields %v), want %q", got, lines, want)
	}
...
func TestPublishResolvedServiceDoesNotPublishWhenEndpointIsNotReady(t *testing.T) {
	...
	logPath := filepath.Join(t.TempDir(), "higgs.log")
	err := publishResolvedService(writeFakeHiggs(t, logPath), manifest)
	...
	if data, readErr := os.ReadFile(logPath); readErr == nil && len(data) != 0 {
		t.Fatalf("Higgs was called despite failed readiness: %q", data)
	}
}

func writeFakeHiggs(t *testing.T, logPath string) string {
	t.Helper()
	path := filepath.Join(t.TempDir(), "higgs")
	script := "#!/bin/sh\nprintf '%s\\n' \"$*\" >> " + logPath + "\n"
	if err := os.WriteFile(path, []byte(script), 0o755); err != nil {
		t.Fatalf("write fake Higgs: %v", err)
	}
	return path
}
