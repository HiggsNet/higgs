...
func TestDaemonStateStoreGossipCheckpointProjectionIsDetached(t *testing.T) {
	store := NewDaemonStateStore(&stateFile{SyncPeers: map[string]syncPeerState{
		"peer.catofes.": {
			BackoffUntilUnix:   20,
			ObservedGraceAddrs: []observedGraceAddrState{{Addr: "192.0.2.1:4242", UntilUnix: 30}},
			RejectedDigests: map[string]rejectedDigestState{"zone:bad.catofes.": {
				Zone: "bad.catofes.", Object: "zone", RootHashHex: hex.EncodeToString([]byte("root")),
			}},
		},
	}})
	checkpoint, report := store.GossipCheckpointProjection()
	if report.PeersMigrated != 1 || report.RejectedMigrated != 1 {
		t.Fatalf("report = %+v", report)
	}
	peer := checkpoint.Peers["peer.catofes."]
	peer.ObservedGraceEndpoints[0].Endpoint = "mutated"
	rejected := peer.RejectedObjects["bad.catofes."]
	rejected.RootHash[0] = 'X'
	peer.RejectedObjects["bad.catofes."] = rejected
	checkpoint.Peers["peer.catofes."] = peer

	fresh, _ := store.GossipCheckpointProjection()
	freshPeer := fresh.Peers["peer.catofes."]
	if freshPeer.ObservedGraceEndpoints[0].Endpoint != "192.0.2.1:4242" || string(freshPeer.RejectedObjects["bad.catofes."].RootHash) != "root" {
		t.Fatalf("projection retained mutable data: %+v", freshPeer)
	}
}

func TestDaemonStateStoreCommonStateProjectionIsPinnedAndDetached(t *testing.T) {
	rootPublic, rootPrivate, err := ed25519.GenerateKey(nil)
	if err != nil {
		t.Fatalf("GenerateKey(root): %v", err)
	}
	authority := &zone.ZoneAuthority{Zone: zone.RootZone, Epoch: 1, Threshold: 1, Keys: []zone.AuthorizedKey{{Key: rootPublic}}}
	network := zone.NewNetworkState()
	network.Zones[zone.RootZone] = zone.NewZoneState(zone.RootZone, authority)
	...
	candidate, report, err := store.CommonStateProjection(rootPublic)
	if err != nil {
		t.Fatalf("CommonStateProjection: %v", err)
	}
	if report.PeersMigrated != 1 || candidate.Verified.ManagedZone != zone.RootZone || candidate.Gossip.Peers["peer.catofes."].BackoffUntilUnix != 20 {
		t.Fatalf("candidate/report = %+v/%+v", candidate, report)
	}
	candidate.Verified.RootPrivateKey[0] ^= 0xff
	candidate.Verified.TrustedRootPublicKey[0] ^= 0xff
	candidate.Verified.Network.Zones[zone.RootZone].Authority.Keys[0].Key[0] ^= 0xff
	peer := candidate.Gossip.Peers["peer.catofes."]
	peer.BackoffUntilUnix = 99
	candidate.Gossip.Peers["peer.catofes."] = peer

	fresh, _, err := store.CommonStateProjection(rootPublic)
	if err != nil {
		t.Fatalf("fresh CommonStateProjection: %v", err)
	}
	if fresh.Verified.RootPrivateKey[0] != rootPrivate[0] || fresh.Verified.TrustedRootPublicKey[0] != rootPublic[0] ||
		fresh.Verified.Network.Zones[zone.RootZone].Authority.Keys[0].Key[0] != rootPublic[0] || fresh.Gossip.Peers["peer.catofes."].BackoffUntilUnix != 20 {
		t.Fatal("common state projection retained mutable output")
	}
	...
	if _, _, err := store.CommonStateProjection(otherPublic); !errors.Is(err, errLegacyCommonStateInvalid) {
		t.Fatalf("wrong root error = %v, want errLegacyCommonStateInvalid", err)
	}
}
