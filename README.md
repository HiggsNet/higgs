...
Phase 4 StrongSwan/IKEv2 + XFRM interface 控制模块正在推进：daemon 已能在 active state 变更后从本地 link group 推导 desired TransportLink，持久化 `LinkInstance`，通过 driver `ListSAs` 观测 adopt 已存在 SA，并用 dry-run apply 验证 create/update/repair/teardown/backoff 路径；daemon 启动进入主循环前也会主动执行一次 IPsec reconcile，从 active state、本地 `LinkGroupSpec`、持久化 `LinkInstance` 和 driver SA 快照恢复 link state，不必等待下一次 record/reload 事件；daemon `reload` control event 会重新读取 `config.yaml`，让 `overlays:`、`connect/deny`、`overlay.default_netns` 等本地 mesh/link group/netns 变化进入同一条 reconcile 路径，且拒绝热切换 state DB/control socket；daemon 事件队列会合并同一轮内多次 state change/config reload，避免同一个 peer/group 短时间重复 reconcile/apply；teardown 成功后会移除本地实例，避免 link group 删除、record 过期或 peer revocation 后重复清理/重建。`higgs debug links` 会重算当前 desired links，并与已落盘 instance、最近 SA/CHILD_SA、XFRM `if_id`、endpoint、backoff 和错误并排显示。显式 `make ipsec-xfrm-smoke` 已作为 root/system integration 入口验证 preflight 和真实 XFRM netns/interface/address 生命周期，但完整双 daemon + VICI IKE_SA/CHILD_SA + tunnel ping 仍是下一步；低频生产可用的平滑端口 rotate 已提前到 Phase 4.4 规划，高频/对抗性 port hopping 留到 Phase 7；WireGuard 后移为可选轻量传输驱动。
