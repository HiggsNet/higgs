...
printf 'ipsec/xfrm smoke passed (preflight + SystemXFRMDriver lifecycle + peer tunnel ping + StrongSwan key/conn load + StrongSwan IKE bring-up + daemon reconcile system apply + daemon StrongSwan/XFRM bring-up + daemon run gossip StrongSwan bring-up)\n'
