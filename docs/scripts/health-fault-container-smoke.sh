...
base_image="${HIGGS_HEALTH_FAULT_IMAGE:-ubuntu:24.04}"
cache_suffix="${base_image//[^A-Za-z0-9_.-]/-}"
cache_image="${HIGGS_HEALTH_FAULT_CACHE_IMAGE:-higgs-health-fault-smoke:${cache_suffix}}"
rebuild_image="${HIGGS_HEALTH_FAULT_REBUILD_IMAGE:-0}"
...
cache_prefix="${HIGGS_HEALTH_FAULT_CACHE_PREFIX:-higgs-health-fault}"
go_cache_volume="${HIGGS_HEALTH_FAULT_GO_CACHE_VOLUME:-${cache_prefix}-gocache}"
go_mod_cache_volume="${HIGGS_HEALTH_FAULT_GO_MOD_CACHE_VOLUME:-${cache_prefix}-gomodcache}"
...
ensure_image() {
  ...
  build_ctx="$(mktemp -d "${TMPDIR:-/tmp}/higgs-health-fault-image.XXXXXX")"
...
"$container_runtime" run --rm -t --privileged \
  ...
  -e HIGGS_HEALTH_FAULT_CONTAINER_SMOKE=1 \
  "$cache_image" bash -lc '
    set -euo pipefail
