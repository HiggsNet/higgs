...
base_image="${PHOTON_HEALTH_FAULT_IMAGE:-ubuntu:24.04}"
cache_suffix="${base_image//[^A-Za-z0-9_.-]/-}"
cache_image="${PHOTON_HEALTH_FAULT_CACHE_IMAGE:-photon-health-fault-smoke:${cache_suffix}}"
rebuild_image="${PHOTON_HEALTH_FAULT_REBUILD_IMAGE:-0}"
...
cache_prefix="${PHOTON_HEALTH_FAULT_CACHE_PREFIX:-photon-health-fault}"
go_cache_volume="${PHOTON_HEALTH_FAULT_GO_CACHE_VOLUME:-${cache_prefix}-gocache}"
go_mod_cache_volume="${PHOTON_HEALTH_FAULT_GO_MOD_CACHE_VOLUME:-${cache_prefix}-gomodcache}"
...
printf '[health-fault-container] runtime=%s base_image=%s cache_image=%s repo=%s\n' "$container_runtime" "$base_image" "$cache_image" "$repo_root"
printf '[health-fault-container] starting disposable privileged container; nested LXC/CI may still block netns/tc/BIRD\n'

ensure_image() {
  if [ "$rebuild_image" != "1" ] && "$container_runtime" image inspect "$cache_image" >/dev/null 2>&1; then
    printf '[health-fault-container] using cached image %s\n' "$cache_image"
    return
  fi

  build_ctx="$(mktemp -d "${TMPDIR:-/tmp}/photon-health-fault-image.XXXXXX")"
...
DOCKERFILE
  ...
  printf '[health-fault-container] building cached image %s from %s\n' "$cache_image" "$base_image"
