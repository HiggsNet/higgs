...
base_image="${HIGGS_IPSEC_XFRM_IMAGE:-ubuntu:24.04}"
cache_suffix="${base_image//[^A-Za-z0-9_.-]/-}"
cache_image="${HIGGS_IPSEC_XFRM_CACHE_IMAGE:-higgs-ipsec-xfrm-smoke:${cache_suffix}}"
rebuild_image="${HIGGS_IPSEC_XFRM_REBUILD_IMAGE:-0}"
...
cache_prefix="${HIGGS_IPSEC_XFRM_CACHE_PREFIX:-higgs-ipsec-xfrm}"
go_cache_volume="${HIGGS_IPSEC_XFRM_GO_CACHE_VOLUME:-${cache_prefix}-gocache}"
go_mod_cache_volume="${HIGGS_IPSEC_XFRM_GO_MOD_CACHE_VOLUME:-${cache_prefix}-gomodcache}"
...
ensure_image() {
  ...
  build_ctx="$(mktemp -d "${TMPDIR:-/tmp}/higgs-ipsec-xfrm-image.XXXXXX")"
...
# HIGGS_CONTAINER_USERNS= to disable it for comparison.
extra_args=()
