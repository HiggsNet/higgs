...
当前实现的 planner/reconcile 核心在 `pkg/transport/ipsec`：`PlanTransportLinks` 从 verified active state 和本地 `LinkGroupSpec` 推导 desired `TransportLinkSpec`，并返回结构化 skip reason；`LinkInstance` 持久化 desired spec hash、实际状态、XFRM if_id、IKE/CHILD_SA 名称、endpoint 与 Higgs owner 信息；`ReconcileLinkInstances` 根据 desired spec、持久化 instance、driver SA 观测和 revocation 输入产生 create/update/adopt/repair/teardown/noop action。daemon 仍需要把这些纯函数接到 state-change hook、实例持久化和 control/debug 输出上。
