...
- [ ] **3.6.8 Catalog sync / bounded UDP control**
  ...
  - [ ] 定义并实现 `CatalogSummary`：`catalog_root`、`zone_count`、可选 bounded `first_page` / `next_cursor`；`PING` / `PONG` 不再承诺携带完整 `ZoneDigest[]`。
  - [ ] 新增 bounded catalog page 消息：`FETCH_CATALOG_PAGE{cursor}` 与 `CATALOG_PAGE{catalog_root, entries[], next_cursor}`；`entries[]` 必须按 `max_datagram_bytes` 打包，单页超预算时 fail closed 并输出诊断。
  - [ ] 同步状态机改为 summary -> catalog page diff -> object pull：catalog root 相同直接完成；root 不同则分页 diff，发现 Zone digest 不同后再 `FETCH_ZONE` / TCP object pull。
    - `SyncSession` 需要拆分当前过宽的 `AwaitingAnnounce`：新增/替代 `SummarySent`、`CatalogDiffing`、`ServingPeerFetch` 等状态，让 `ANNOUNCE` 只作为 wakeup/hint 或小 payload 优化。
    - 新增 catalog 事件/action：`CatalogSummaryReceivedEvent`、`CatalogPageReceivedEvent`、`CatalogPageTimeoutEvent`、`SendFetchCatalogPageAction`、`SendCatalogPageAction`；`ObjectPulling` / `ChunkFallback` 继续作为完整对象传输阶段。
    - `PacketQuietTimeout` 只用于 UDP hint/page quiet 和 fallback 收尾，不能再作为“发现 digest mismatch 后才启动 object pull”的主路径；page diff 得出的不同 Zone 应立即进入 object pull。
