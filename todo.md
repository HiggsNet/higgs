...
**当前建议顺序：**
1. **7.1 模型决策与验证性实验已冻结**：BIRD 双接口、WG/GRE 基础数据路径与 staged rotate 均已在真实 root/netns lane 通过；下一步按双 provider 边界实现 WG/GRE，并收口共同的 Babel-facing link 输出。
...
- [ ] **7.1 异构 TransportLink 并行共存（模型已冻结，待验证与实现）**
  ...
  - [ ] **7.1.e 公共 LinkOutput 与消费者收口**：定义只包含 peer/group/provider、netns/interface/address/MTU/cost、generation/runtime role 和分层 readiness 的 Babel-facing `LinkOutput`。StrongSwan state 与 WG/GRE state 分别投影到该输出；routing/firewall/health/inspect 消费聚合结果，不读取 provider 内部 action/resource。
  - [ ] **7.1.f WG/GRE 正式 provider**：先一次性落地共享的 provider-aware Link ID helper，并为 StrongSwan legacy ID adopt 增加 planner/state 迁移测试；再实现 provider-specific records/policy、device/peer/GRE desired state、独立持久化 state、owner marker、inspect/reconcile/apply/teardown、shared-device staged rotate 和零引用 cleanup。不迁移或包装 StrongSwan 内部 lifecycle。
  - [ ] **7.1.g BIRD/readmodel/runtime 联合收口**：per-interface policy blocks、按 peer/group/provider 展示多 link、双 provider dry-run，以及 IPsec/XFRM + WG/GRE 联合 root/container smoke。
...
1. 进入 7.1.e 公共 `LinkOutput` 与消费者收口，再实现 7.1.f WG/GRE 正式 provider；StrongSwan 主链路保持现状，不再迁移到通用 desired/runtime/action 模型。
