...
- [ ] **6.7.7 `app/higgs` 模块化重构（Observer/debug 先行）**
  ...
  - [ ] Diagnostics/debug 迁移路线：
    ...
    - `app/higgs/debug_firewall.go`、`debug_revoke_impact.go` 已完成 presenter 迁移；后续只保留系统 apply/reconcile、control socket/offline fallback 和 app 私有 runtime adapter。
      - 已新增 `inspect.BuildFirewallDebug` / `FirewallDebugInput`：firewall config + reconcile snapshot 到 debug view 的转换下沉到 inspect；app 层仅保留 control socket/offline fallback 与私有 config/snapshot 投影。
