  ...
  - [ ] **7.10.1 Daemon state 读写分离 / committed snapshot 重构（当前 P0）**
    ...
    - **实施顺序：**
      ...
      - [ ] 最后移除 `DaemonService.stateUnlock` / `lockState()` 这类 live pointer 锁追踪，daemon event loop 只通过 StateStore 操作。
        - 已移除 `stateUnlock func()` 的 unlock 函数追踪，改为局部锁闭包 + `lockedState` 指针转移，避免保存可调用 unlock；`setState` 仍保持显式 live-lock 转移语义。object pull result / submit attempt / immediate failure stats 已改为 StateStore 短事务提交 committed snapshot，并避免在 live lock 持有期间安装新 state 指针；sync FSM/responder 的 catalog、active-pull、hint、rejected digest、backoff、peer-sync、read-only responder、datagram、relay suppression/success 诊断也已同步写入 committed snapshot；daemon sync 的 `ApplySnapshotAction` 已在 Store workspace 内 apply Network snapshot / rejected digest / auto-join 诊断并锁内切换到 committed state，`SaveStateAction`、apply 后保存、relay session 保存也改为保存 committed state；`record_put`、delegate issue、authority grant、delegate revoke、recovery import、recovery purge apply、IPsec port rotate、IPsec cleanup、endpoint timer 已迁到 Store workspace 修改并保存 committed state，且 live lock 持有期间不会用旧 live pointer 覆盖 committed snapshot；daemon event loop 已移除外层 `lockState()`，`record_put` 等 store 写事件不会被 live state 锁阻塞，sync FSM 事件处理也已移除内部 `lockState()`，catalog/filter/reconcile 输入改为读取 committed snapshot；sync timer、read-only responder、hinted session、relay fanout、object-pull submit、daemon zone digest 和 IPsec interval 判断也已迁到 committed snapshot 输入；`runStateStoreWrite` / `record_put` / recovery purge / IPsec cleanup 的旧 `StateStore == nil` live-write fallback 已收缩为初始化错误，object-pull attempt/result 和 sync snapshot apply 的旧 live fallback 已清掉；staticcheck 标出的旧 helper / live wrapper 已清理。后续剩余主要是继续收缩 reconcile/helper 的 live-state guard，并逐步弱化测试专用 `lockState()`。
